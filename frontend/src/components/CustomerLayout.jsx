...
import {
  LayoutDashboard, Package, Bell, User, Utensils, ShoppingCart,
  Home, Sun, Moon, LogOut, ChevronDown, Menu,
  ChefHat,
} from "lucide-react";
...
export default function CustomerLayout() {
  ...
  const [openGroups, setOpenGroups] = useState({ overview: true, account: true, shopping: true });
  ...
  const pageLabel = () => {
    ...
    if (location.pathname.startsWith("/restaurants/")) return "Restaurant Details";
    return "My Account";
  };
        ...
        {GROUPS.map(group => {
          ...
          return (
            <div key={group.id} className="pt-1">
              <button onClick={() => setOpenGroups(prev => ({ ...prev, [group.id]: !prev[group.id] }))}
              ...
              {isOpen && (
                <div className="space-y-0.5 ml-1">
                  {group.items.map(({ to, label, icon: Icon, exact, badge, cart }) => (
                    <Link key={to} to={to} onClick={() => setSidebarOpen(false)}
                      ...
                      {label}
                      {badge && unreadCount > 0 && (
                         <span className={clsx("ml-auto w-3.5 h-3.5 rounded-full flex items-center justify-center text-[8px] font-bold",
                           isActive({ to, exact: exact ?? false }) ? "bg-white/25 text-white" : "bg-red-500 text-white")}>
                          {unreadCount > 9 ? "9+" : unreadCount}
                        </span>
                      )}
                      {cart && totalItems > 0 && (
                        <span className="ml-auto w-3.5 h-3.5 rounded-full flex items-center justify-center text-[8px] font-bold bg-primary-500 text-white">{totalItems > 9 ? "9+" : totalItems}</span>
                      )}
                    </Link>
                  ))}
                </div>
        ...
        <header className="py-4 bg-white dark:bg-stone-950 border-b border-cream-300 dark:border-stone-800 flex items-center justify-between px-4 sm:px-6 shrink-0">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <Link to="/cart" className="relative p-2 rounded-lg text-stone-400 hover:text-stone-700 dark:hover:text-stone-200 hover:bg-cream-200 dark:hover:bg-stone-800 transition-all">
              ...
              {totalItems > 0 && <span className="absolute -top-0.5 -right-0.5 bg-primary-500 text-white text-[8px] w-3.5 h-3.5 rounded-full flex items-center justify-center font-bold">{totalItems > 9 ? "9+" : totalItems}</span>}
            </Link>
