...
export default function Sidebar({ items, bottomItems, title, subtitle }) {
  ...
  const renderIcon = (IconOrStr) => {
    ...
    if (typeof IconOrStr === "string")
      return <span className="text-base">{IconOrStr}</span>;
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        {items?.map((item) => (
          ...
          >
            ...
            {item.badge > 0 && (
              <span className="ml-auto bg-white/25 text-white text-[8px] w-3.5 h-3.5 rounded-full flex items-center justify-center font-bold">
