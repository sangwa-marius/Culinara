...
export default function Navbar() {
  ...
  const { notifications, unreadCount, markOneRead, markAllRead } = useNotificationContext();
  ...
  useEffect(() => { setNotifOpen(false); setDropOpen(false); setMenuOpen(false); }, [location.pathname]);
    ...
    <nav className={clsx(
            ...
            <>
              ...
              <div className="relative">
                <button onClick={() => { setNotifOpen(!notifOpen); setDropOpen(false); }}
                  className={clsx(
                    "relative p-2.5 rounded-lg transition-all",
                    unreadCount > 0 ? "text-primary-500" : transparent ? "text-white/70 hover:text-white hover:bg-white/10" : "text-stone-500 hover:text-stone-900 dark:text-stone-400 hover:bg-cream-200 dark:hover:bg-stone-800"
                  )}>
                  <Bell size={18} />
                  {unreadCount > 0 && (
                  ...
                  <div className="absolute right-0 top-[calc(100%+8px)] w-80 card animate-slide-down shadow-xl z-50">
                    ...
                    <div className="max-h-72 overflow-y-auto">
                      {notifications.length === 0 ? (
                        <p className="py-8 text-center text-stone-400 text-sm">No notifications yet</p>
                      ) : notifications.slice(0, 8).map((n) => (
                        <div key={n._id} onClick={() => { if (!n.isRead) markOneRead(n._id); if (n.orderId) { navigate(`/orders/${n.orderId}`); setNotifOpen(false); } }}
                          className={clsx("px-4 py-3 border-b border-cream-200 dark:border-stone-800 last:border-0 cursor-pointer hover:bg-cream-100 dark:hover:bg-stone-800 transition-colors flex gap-3", !n.isRead && "bg-primary-50/30 dark:bg-primary-950/20")}>
                          <span className="text-lg shrink-0">{{ order: "📦", promo: "🎉", system: "🔔", delivery_request: "🛵" }[n.type] || "🔔"}</span>
                          <div className="flex-1 min-w-0">
                            <p className={clsx("text-xs leading-snug", n.isRead ? "text-stone-500" : "text-stone-800 dark:text-stone-100 font-medium")}>{n.message}</p>
                            <p className="text-[10px] text-stone-400 mt-0.5">
                              {(() => { try { return formatDistanceToNow(new Date(n.createdAt), { addSuffix: true }); } catch { return "just now"; } })()}
                            </p>
                          </div>
                          {!n.isRead && <div className="w-1.5 h-1.5 rounded-full bg-primary-500 mt-1.5 shrink-0" />}
                        </div>
                      ))}
                    </div>
                    <div className="px-4 py-2.5 border-t border-cream-300 dark:border-stone-700">
                      <Link to="/notifications" onClick={() => setNotifOpen(false)} className="text-xs text-primary-500 font-semibold flex items-center justify-center gap-1 hover:text-primary-600">
              ...
              <div className="relative ml-1">
                <button onClick={() => { setDropOpen(!dropOpen); setNotifOpen(false); }}
                  ...
                  <ChevronDown size={13} className={clsx(dropOpen && "rotate-180", "transition-transform hidden md:block", transparent ? "text-white/60" : "text-stone-400")} />
                </button>
                ...
                {dropOpen && (
                  <div className="absolute right-0 top-[calc(100%+8px)] w-52 card animate-slide-down shadow-xl z-50">
                    <div className="px-4 py-3 border-b border-cream-300 dark:border-stone-700">
                      <p className="font-semibold text-stone-900 dark:text-white text-sm truncate">{user.name}</p>
                      <p className="text-xs text-stone-400 capitalize">{user.role?.replace(/_/g, " ")}</p>
                    </div>
                    <div className="py-1">
                      {[
                        { to: getDashLink(), icon: <LayoutDashboard size={14} />, label: "Dashboard" },
                        { to: "/orders",     icon: <Package       size={14} />, label: "My Orders" },
                        { to: "/profile",    icon: <Settings      size={14} />, label: "Profile" },
                      ].map(({ to, icon, label }) => (
                        <Link key={to} to={to} className="flex items-center gap-3 px-4 py-2.5 text-sm text-stone-600 dark:text-stone-300 hover:bg-cream-100 dark:hover:bg-stone-800 transition-colors">
                          <span className="text-stone-400">{icon}</span>{label}
                        </Link>
                      ))}
                      <div className="my-1 border-t border-cream-300 dark:border-stone-700" />
                      <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 w-full transition-colors">
      ...
      {menuOpen && (
        <div className={clsx("md:hidden border-t px-4 py-3 space-y-1 animate-slide-down",
          ...
          {user && (
            <button onClick={handleLogout} className="flex items-center gap-2 px-3 py-2.5 text-sm text-red-500 w-full rounded-lg hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors">
              <LogOut size={14} /> Sign Out
            </button>
          )}
          <div className="pt-1 border-t border-cream-300 dark:border-stone-700">
            <button onClick={toggleTheme} className="flex items-center gap-2 px-3 py-2.5 text-sm text-stone-600 dark:text-stone-300 w-full rounded-lg hover:bg-cream-200 dark:hover:bg-stone-800 transition-colors">
